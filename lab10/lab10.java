...
public class lab10 {
    ...
    private static void modifyStringInternal(String target, Field valueField, String replacement)
        ...
        if (internal instanceof char[]) {
            ...
            for (int i = 0; i < len; i++)
                arr[i] = repl[i];
            for (int i = repl.length; i < arr.length; i++)
                arr[i] = '\0';
            ...
            if (repl.length < arr.length)
                Arrays.fill(arr, repl.length, arr.length, (byte) 0);
            debug("Replaced byte[] in String (compact string mode)");
        } else {
            warn("Unknown String internal representation: %s",
                    internal == null ? "null" : internal.getClass().getName());
        }
    }
    ...
    private static void runBankTest(ResourceBundle rb) throws InterruptedException {
        ...
        System.out.println(rb.getString("bank.result") + " "
                + (initialSum == finalSum ? rb.getString("bank.passed") : rb.getString("bank.failed")));
    }
            ...
            return new ListResourceBundle() {
                protected Object[][] getContents() {
                    return new Object[][] {
                            ...
                            { "menu.option.reflection",
                                    "1. Демонстрація рефлексії (зміна внутрішнього представлення String)" },
